	...
	public:
		...
		void change(char* infix, char* output) // 中序轉後序的函數
		{
			...
			{								   // \0字串結束字元
				...
				{
					while (item[top] != '(') // 只要不遇到左括號
					{
						output[j++] = item[top--]; // 將 stack 裡的數 pop 出來，將 top--(把 pop 出來的數從 stack 移除)
					}
					item[top--]; // 把 '(' 從 stack 裡刪除
				}
				else // 如果 val是運算元
				{
					while (top > -1) // 如果 stack裡有東西
					{
						if (order(val) > order(item[top])) // 若 val 的優先權大於 stack 的 top
						{
							break; // 跳出此 while迴圈
						}
						else // 若 val 的優先權較小
					...
					item[++top] = val; // 如果 stack 裡原本沒東西或是 val 的優先權較大
					                   // 就 push val 到 stack
				}
			}

			while (top > -1) // stack 裡剩下的
			{
				output[j++] = item[top--]; // 將 stack 裡的數 pop 出來，將 top--(把 pop 出來的數從 stack 移除)
			}
...
int main()
{
	...
	S.change(input, output); // 執行中序轉後序的函式

	cout << "後序表示法結果為：" << endl;

	for (j = 0; j <= strlen(output); j++) // strlen(output)是回傳的字串的長度
	{
